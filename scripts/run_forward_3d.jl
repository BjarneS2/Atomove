...
function load_protocol3d_and_params(path::String)
    h5open(path, "r") do file
        ...
        println("\n┌─────────────────────────────────────────┐")
        println("│           Loaded TweezerParams3D        │")
        println("├──────────────────────────┬──────────────┤")
        for (name, val) in [
            ("w", params.w),
            ...
            ("final_trap_fraction", params.final_trap_fraction),
...
function apply_stress_test_overrides(params::TweezerParams3D)
    trap_fraction = ENABLE_TRAP_FRACTION_CONSTRAINT ? params.trap_fraction : nothing
    T_atom = T_ATOM_OVERRIDE === nothing ? params.T_atom : T_ATOM_OVERRIDE
    final_trap_fraction = FINAL_TRAP_FRACTION_OVERRIDE === nothing ? params.final_trap_fraction : FINAL_TRAP_FRACTION_OVERRIDE

    if !ENABLE_TRAP_FRACTION_CONSTRAINT || T_ATOM_OVERRIDE !== nothing || FINAL_TRAP_FRACTION_OVERRIDE !== nothing
        println("\n── Stress-test overrides applied ──")
        ENABLE_TRAP_FRACTION_CONSTRAINT || println("  trap_fraction constraint: DISABLED (was $(params.trap_fraction))")
        T_ATOM_OVERRIDE === nothing || @printf("  T_atom: %.3g K -> %.3g K\n", params.T_atom, T_atom)
        FINAL_TRAP_FRACTION_OVERRIDE === nothing || @printf("  final_trap_fraction: %.3g -> %.3g\n", params.final_trap_fraction, final_trap_fraction)
    end
...
function main()
    results_dir = joinpath(@__DIR__, "..", "ResultsForThesis")
    protocol_file = length(ARGS) >= 1 ? ARGS[1] :
                    (PROTOCOL_FILE_OVERRIDE !== nothing ? PROTOCOL_FILE_OVERRIDE : latest_protocol_file(results_dir))
    ...
    params = apply_stress_test_overrides(params)
