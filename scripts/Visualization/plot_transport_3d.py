...
fixed   : camera locked to the initial angle/distance for every frame — no
          orbit, no fly-in, no azimuth sweep, so matplotlib never re-orders or
          flips the axes mid-animation.
...
def create_transport_animation_3d(data: Dict, output_dir: Path,
    ...
    camera_mode      : "follow" — camera tracks aux tweezer.
                       ...
                       "fixed"  — camera locked to the initial angle/distance
                                  on every frame (no movement at all), so the
                                  axes never flip or re-order.
    """
    t       = data["t"]
    x       = data["x"]
    y       = data["y"]
    ...
    # "fixed" camera never moves, so the hold/fly choreography is just dead time.
    hold_frames  = 0 if camera_mode == "fixed" else int(fps * hold_s)
    remaining    = total_frames - hold_frames
    fly_frames   = 0 if camera_mode == "fixed" else max(1, int(remaining * FLY_FRAC))
    ...
    def update(frame_idx):
        ...
        if camera_mode == "fixed":
            ax.view_init(elev=elev_near, azim=follow_azim[0])  # type: ignore[attr-defined]
            _set_dist(dist_near)
        ...
        # re-apply fixed limits every frame — matplotlib resets them after
        # plot_surface / view_init calls
        ax.autoscale(False)
        _fix_limits()
