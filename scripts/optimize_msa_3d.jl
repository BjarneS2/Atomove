...
params = TweezerParams3D(
    ...
    T_atom         = T_atom,
    lambda_heat        = 0.0,
    lambda_jitter_pos  = 0.0,
    lambda_jitter_ua   = 0.0,
    starting_trap_fraction = 0.8,
    trap_fraction          = 0.4,
    final_trap_fraction    = 0.4,
)

msa_config = MSAConfig3D(
    N_modes_s      = 10,
    N_modes_ua     = 1,
    ua_mode        = :square,
    lambda_slosh   = 1.0,
...
filename  = joinpath(results_dir, "control3d_msa_$(timestamp).h5")
...
h5open(filename, "w") do file
    ...
    attrs(file)["N_modes_s"]         = msa_config.N_modes_s
    attrs(file)["N_modes_ua"]        = msa_config.N_modes_ua
    attrs(file)["lambda_slosh"]      = msa_config.lambda_slosh
    attrs(file)["lambda_mse"]        = msa_config.lambda_mse
    attrs(file)["use_primary"]       = Int(msa_config.use_primary)
    attrs(file)["use_secondary"]     = Int(msa_config.use_secondary)
    ...
    write(file, "coeff_s",  result.coeff_s)
    write(file, "coeff_ua", result.coeff_ua)
    attrs(file)["delta_s0"] = result.delta_s0
