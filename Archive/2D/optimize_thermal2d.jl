...
# Choose time unit so that E0 = kB*T_tweezer, making U0_static = 1 exactly.
# v0 = sqrt(kB*T_tweezer/m)  →  t0 = w0_SI / v0
let _c = default_constants2d()
    _v0 = sqrt(_c.kB * T_tweezer / _c.m)
    global consts = PhysicalConstants2D(; t0_us = (_c.w0_um * 1e-6) / _v0 * 1e6)
end
...
v0    = w0_SI / t0_SI          # ≈ sqrt(kB*T_tweezer/m)
E0    = m * v0^2               # ≈ kB*T_tweezer  (by construction)

U0_static  = 1.0               # kB*T_tweezer / E0 = 1 by construction
ua_max     = 280/180
U0_aux_max = ua_max            # = ua_max * U0_static = ua_max * 1
...
params = TweezerParams2D(
    ...
    z_stop         = z_stop,
    n              = 1001,
    maxT           = 500.0 / consts.t0_us,   # 500 μs in new time units
    U0_static      = U0_static,
    U0_aux_max     = U0_aux_max,
    T_tweezer      = T_tweezer,
    T_atom         = T_atom,
    lambda_jitter_pos = 1.0,
    lambda_jitter_ua  = 1.0,
...
bounds = ControlBounds2D(
    ...
    ua_max          = ua_max,
    v_u_max_per_w   = v_max,
    v_ua_max        = 26.0 * ua_max * consts.t0_us,   # same physical slew rate, rescaled to new t0
)
...
h5open(filename, "w") do file
    ...
    write(file, "guess_ux", guess.ux)
    write(file, "guess_uz", guess.uz)
    write(file, "guess_ua", guess.ua)
