...
function optimize_controls3d_single(
    ...
    if guess !== nothing
        ...
        if n_g == n
            for j in 1:n
                if !linear_r
                    s_guess = abs(ex) >= abs(ey) ? (guess.ux[j] - p.x_start) / ex :
            ...
            for j in 1:n
                if !linear_r
                    s_guess = abs(ex) >= abs(ey) ? (interp_guess(guess.ux, j) - p.x_start) / ex :
                              (interp_guess(guess.uy, j) - p.y_start) / ey
                    set_start_value(s[j], clamp(s_guess, 0.0, L))
                end
                set_start_value(ua[j], interp_guess(guess.ua, j))
                set_start_value(x[j], interp_guess(guess.x, j))
                set_start_value(y[j], interp_guess(guess.y, j))
                set_start_value(z[j], interp_guess(guess.z, j))
                set_start_value(vx[j], interp_guess(guess.vx, j))
                set_start_value(vy[j], interp_guess(guess.vy, j))
                set_start_value(vz[j], interp_guess(guess.vz, j))
            end
        end
    else
    ...
    @expression(model, Fy_beam[j=1:n],
    ...
    @expression(model, Fy_total[j=1:n], Fy_beam[j] - g)
    @expression(model, Fz[j=1:n],
    ...
    for j in 1:(n-1)
        ...
        @constraint(model, z[j+1] - z[j] == vz[j]*dt + 0.5*Fz[j]*dt^2)
        @constraint(model, vx[j+1] - vx[j] == 0.5*dt*(Fx[j] + Fx[j+1]))
        @constraint(model, vy[j+1] - vy[j] == 0.5*dt*(Fy_total[j] + Fy_total[j+1]))
    ...
    @expression(model, E_tot[j=1:n], KE[j] + U_tot[j] + g*y[j])
