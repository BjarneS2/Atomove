...
function optimize_controls3d_thermal(
    ...
    linear_solver::String="spral",
    fix_final_pos::Bool=false,
)
    ...
    wa2 = w_a^2
    cz = 0.0
    static_factor = p.move_in_single_trap ? 0.0 : 1.0
    ...
    if guess !== nothing
        ...
        for j in 1:n
            s_g = ex != 0.0 ? (ux_n[j] - p.x_start) / ex : (uy_n[j] - p.y_start) / ey
            set_start_value(s[j], clamp(s_g, 0.0, L))
            set_start_value(ua[j], ua_n[j])
        end

        for i in 1:n_samples, j in 1:n
            set_start_value(x_s[i, j], state_guess[i].x[j])
            set_start_value(y_s[i, j], state_guess[i].y[j])
            set_start_value(z_s[i, j], state_guess[i].z[j])
            set_start_value(vx_s[i, j], state_guess[i].vx[j])
            set_start_value(vy_s[i, j], state_guess[i].vy[j])
            set_start_value(vz_s[i, j], state_guess[i].vz[j])
        end
    else
        set_start_value(T, 0.5 * p.maxT)
        for j in 1:n
            set_start_value(s[j], (j-1)/(n-1) * L)
            set_start_value(ua[j], p.move_in_single_trap ? (p.single_trap_amplitude * p.U0_static / p.U0_aux_max) : 0.0)
        end
    ...
    @constraint(model, ua[1] == ua_bc)
    @constraint(model, ua[n] == ua_bc)
    ...
    for i in 1:n_samples
        ...
        if fix_final_pos
            @constraint(model, x_s[i, n] == p.x_stop)
            @constraint(model, y_s[i, n] == p.y_stop)
        else
    ...
    @expression(model, Fx_s[i=1:n_samples, j=1:n],
        -4.0*static_factor*p.U0_static*(x_s[i, j]-p.x_start)/wXi1_s[i, j]*f1_s[i, j]
        -
        4.0*static_factor*p.U0_static*(x_s[i, j]-p.x_stop) / wXi2_s[i, j]*f2_s[i, j]
        -
        4.0*ua[j]*p.U0_aux_max*(x_s[i, j]-ux[j])/wXia_s[i, j]*fa_s[i, j]
    )
    @expression(model, Fy_beam_s[i=1:n_samples, j=1:n],
        -4.0*static_factor*p.U0_static*y_s[i, j]/(wXi1_s[i, j])*f1_s[i, j]
        -
        4.0*static_factor*p.U0_static*(y_s[i, j]-p.y_stop)/wXi2_s[i, j]*f2_s[i, j]
        -
    ...
    @expression(model, Fz_s[i=1:n_samples, j=1:n],
        static_factor * p.U0_static * f1_s[i, j] * (w2/wXi1_s[i, j]) * (Xi_s[i, j]/zR^2) * (4.0*r1sq_s[i, j]/wXi1_s[i, j]-2.0)
        + static_factor * p.U0_static * f2_s[i, j] * (w2/wXi2_s[i, j]) * (Xi_s[i, j]/zR^2) * (4.0*r2sq_s[i, j]/wXi2_s[i, j]-2.0)
